...
test.describe("Property page", () => {
  test("should display only booked properties", async ({ page }) => {
    ...
    await expect(page.getByRole('img', { name: mockedProperties.properties[0].title})).toBeVisible();
    await expect(page.getByRole('heading', { name: mockedProperties.properties[0].title})).toBeVisible();
    await expect(page.getByText(mockedProperties.properties[0].description)).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Book this property' })).toBeVisible();
    await expect(page.getByTestId("date-range-input").getByRole("button")).toBeVisible();
    await expect(page.getByRole('button', { name: 'Book this property', exact: true })).toBeVisible();
    await expect(page.getByRole('link', { name: '< Back' })).toBeVisible();
